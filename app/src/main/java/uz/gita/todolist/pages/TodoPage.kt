...
class TodoPage : Fragment (R.layout.page_to_to) {
    private val data = ArrayList<TaskEntity>()
    private val taskDao = TaskDatabase.getDatabase().getDao()
    private val adapter by lazy { MainAdapter(data, R.drawable.ic_todo,ContextCompat.getColor(requireContext(), R.color.todo_color), ContextCompat.getColor(requireContext(), R.color.todo_bg)) }
    ...
    fun loadData(){
        data.clear()
        data.addAll(taskDao.getDataByPosition(0))
        backgroundNotes(data)
    ...
    fun backgroundNotes (data : ArrayList<TaskEntity>){
        view?.let {view->
            val noText= view.findViewById<LottieAnimationView>(R.id.noNotesToDo)
            if (data.size == 0){
